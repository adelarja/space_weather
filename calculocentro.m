function [bx_nube,by_nube,bz_nube,date_,theta,phi,R,bx_nube_ext,by_nube_ext,bz_nube_ext,date_ext,eval1,eval2,eval3,evec1,evec2,evec3,B_,B,cociente,cocientefrente,cocienteback,valordelafechaenc]=calculocentro(DIR_WORK,bx_gse_ext_,by_gse_ext_,bz_gse_ext_,date_ext_,Vsw,initial_date,end_date,initial_date_mv,end_date_mv,graf,gap,B__,...
kind_mv,initial_date_graf,end_date_graf,valordelafechaenfrente,valordelafechaenback,jj)
...
if gap>0,
   ...
   bz_gse_ext_<gap-epsilon & date_ext_>=initial_date_mv & date_ext_<=end_date_mv);
elseif gap<0,
   cond_good=find(bx_gse_ext_>gap+epsilon & by_gse_ext_>gap+epsilon & ...
   bz_gse_ext_>gap+epsilon & date_ext_>=initial_date_mv & date_ext_<=end_date_mv);
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear bx_nube by_nube_bz_nube bx by bz
...
%  indicecero=max(find(funciondelflujo==0)&(fechaenh>fechaenh(round(length(fechaenh)/2))))
...
% valordelafechaenfrente=datestr(date_ext(cond_graf(1))+(fechaenfrente/24))
 %valordelafechaenback=datestr(date_ext(cond_graf(1))+(fechaenback/24))
  ...
  indicecero=max(find(funciondelflujo>-20 & funciondelflujo<20))
fechaencero=fechaenh(indicecero)
...
% valordelafechaenc=valordelafecha
% fenc=0;
% fenc=(datenum(valordelafechaenc)-date_ext(cond_graf(1)))*24
  %No---valordelafecha=datestr(date_ext(cond_graf(1))+(fechaenhmax/24))
...
%     rango=find(fechaenh<fechaencero);
%  maximovalor=max(abs(funciondelflujo(rango)));
% indicemaximovalor=find(abs(funciondelflujo(rango))==maximo);
%fechaenhmaximovalor=(fechaenh(indicemaximovalor))
% valordelafechaenc=0; 
% valordelafecha=datestr(date_ext(cond_graf(1))+(fechaenhmaximovalor/24))
% valordelafechaenc=valordelafecha
% fenc=0;
% fenc=(datenum(valordelafechaenc)-date_ext(cond_graf(1)))*24
...
% valordelafechaenc=0; 
% valordelafecha=datestr(date_ext(cond_graf(1))+(fechaenhminsinmodulo/24))
% valordelafechaenc=valordelafecha
% fenc=0;
% fenc=(datenum(valordelafechaenc)-date_ext(cond_graf(1)))*24
 %valordelafecha=datestr(date_ext(cond_graf(1))+(fechaenhminsinmodulo/24))
  ...
  rango=find(fechaenh<fechaencero);
  maximovalor=max(abs(funciondelflujo(rango)));
 indicemaximovalor=min(find(abs(funciondelflujo)==maximovalor));
 fechaenhmaximovalor=(fechaenh(indicemaximovalor))
 valordelafechaenc=0; 
 valordelafecha=datestr(date_ext(cond_graf(1))+(fechaenhmaximovalor/24))
 valordelafechaenc=valordelafecha
...
bandera=0
if bandera==0
 % plot(fechaenh,funciondelflujo,'-.')
%  hold on
%  plot(fechaenhy,funciondey,'.','MarkerSize',graf)
 % hold on
% plot(fechaenhy,funciondelflujo_ext1,'.r','MarkerSize',graf)
%   hold on
%  plot(fechaenhy,funciondelflujo_ext2,'.c','MarkerSize',graf)
%  hold on
% plot(fechaenhy,funciondelflujo_extnueva,'.k','MarkerSize',graf)
%  hold on
%  plot(fechaenhy(indicedelfinenhy),funciondelflujo_ext1(indicedelfinenhy),'hk','MarkerSize',10)
 
    
  % figure
   %subplot(4,1,2)
  % plot((date_ext(cond_graf)-date_ext(cond_graf(1)))*24,...
  % by_nube_ext(cond_graf).*B(cond_graf),'.','MarkerSize',graf)
  % hold on
  % plot((date_-date_ext(cond_graf(1)))*24,Fy/(max(abs(Fy)))*mean(B(cond_graf)),'-.')
  ...
  %  print('-depsc',strcat(DIR_WORK(2:end),'ByMV'))


   % bz
  figure
      plot((date_ext(cond_graf)-date_ext(cond_graf(1)))*24,...
   bz_nube_ext(cond_graf).*B(cond_graf),'.','MarkerSize',graf)
   axis([0 (date_ext(cond_graf(end))-date_ext(cond_graf(1)))*24 bmin bmax])
   line([(initial_date-date_ext(cond_graf(1)))*24,(initial_date-date_ext(cond_graf(1)))*24],...
   [bmin,bmax],'LineStyle','--','Color','k')
   line([(end_date-date_ext(cond_graf(1)))*24,(end_date-date_ext(cond_graf(1)))*24],...
   ...
   ylabel('B_{z,MV} (nT)')
   xlabel(strcat('Time (Hours), after:',datestr(date_ext(cond_graf(1)))))
    print('-depsc',strcat(DIR_WORK(2:end),'BzMV'))
  ...
  % figure
   %subplot(4,1,1)
  % plot((date_ext(cond_graf)-date_ext(cond_graf(1)))*24,...
  % bx_nube_ext(cond_graf).*B(cond_graf),'.','MarkerSize',graf)
  % axis([0 (date_ext(cond_graf(end))-date_ext(cond_graf(1)))*24 bmin bmax])
  % line([(initial_date-date_ext(cond_graf(1)))*24,(initial_date-date_ext(cond_graf(1)))*24],...
  % [bmin,bmax],'LineStyle','--','Color','k')
  % line([(end_date-date_ext(cond_graf(1)))*24,(end_date-date_ext(cond_graf(1)))*24],...
  ...
  % ylabel('B_{x,MV} (nT)')
  % xlabel(strcat('Time (Hours), after:',datestr(date_ext(cond_graf(1)))))
  % print('-depsc',strcat(DIR_WORK(2:end),'BxMV'))
...
%B
   %subplot(4,1,4)
   plot((date_ext(cond_graf)-date_ext(cond_graf(1)))*24,real_mod_B,'.','MarkerSize',graf)
   axis([0 (date_ext(cond_graf(end))-date_ext(cond_graf(1)))*24 0 max(real_mod_B)])
   line([(initial_date-date_ext(cond_graf(1)))*24,(initial_date-date_ext(cond_graf(1)))*24],...
   [bmin,bmax],'LineStyle','--','Color','k')
   line([(end_date-date_ext(cond_graf(1)))*24,(end_date-date_ext(cond_graf(1)))*24],...
   [bmin,bmax],'LineStyle','--','Color','k')
   ...
   ylabel('B (nT)')
   xlabel(strcat('Time (Hours), after:',datestr(date_ext(cond_graf(1)))))
    print('-depsc',strcat(DIR_WORK(2:end),'BMV'))
...
%  plot(fechaenh,funciondelflujo,'-.')
  hold on
  plot(fechaenhy,funciondey,'.','MarkerSize',graf)
 %  plot(fechaenhmax,funciondelflujo(indicemax),'s','MarkerFaceColor','b','MarkerSize',10)
   hold on
  % plot(fechaenhmin,funciondelflujo(indicemin),'d','MarkerFaceColor','b','MarkerSize',10)
   hold on
  % plot(fechaenhmaxsinmodulo,funciondelflujo(indicemaxsinmodulo),'s','MarkerFaceColor','c','MarkerSize',10)
   hold on
  % plot(fechaenhminsinmodulo,funciondelflujo(indiceminsinmodulo),'d','MarkerFaceColor','r','MarkerSize',10)
   hold on
   % plot(fechaenfin,funciondelflujo(indicefin),'d','MarkerFaceColor','g','MarkerSize',10)
     hold on
%  plot(fechaenhy,funciondelflujo_ext2,'.c','MarkerSize',graf)
% hold on
 % plot(fechaenfrente,funcionenfrente,'h','MarkerFaceColor','k','MarkerSize',10)
 %hold on
 % plot(fechaenback,funcionenback,'h','MarkerFaceColor','r','MarkerSize',10)
 %plot(fechaenhy,funciondelflujo_extnueva,'.k','MarkerSize',graf)
 plot(fechaenhy,funciondelflujo_extnueva,'k','LineWidth',4)
...
%  print('-depsc',strcat(DIR_WORK(2:end),'ByMV'))
print('-djpeg',strcat(DIR_WORK(2:end),'ByMV'))
...
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   ...
   plot(fenc,0,'h','MarkerFaceColor','r','MarkerSize',10)
   hold on
   plot(fechaenhmaxsinmodulo,funciondelflujo(indicemaxsinmodulo),'s','MarkerFaceColor','c','MarkerSize',10)
   hold on
  % plot(fechaenhminsinmodulo,funciondelflujo(indiceminsinmodulo),'d','MarkerFaceColor','r','MarkerSize',10)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Distance.
Delta_t=(initial_date-end_date)*24*3600;
